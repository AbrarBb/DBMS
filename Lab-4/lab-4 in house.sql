...
SELECT CHR(65) FROM Customer;

SELECT CONCAT('Tech on', ' the Net') FROM DUAL;

SELECT CONCAT(CONCAT(CUSTOMER_STREET,', '),CUSTOMER_CITY) AS CUSTOMER_ADDRESS FROM CUSTOMER;

SELECT CUSTOMER_STREET || ', ' || CUSTOMER_CITY  AS CUSTOMER_ADDRESS FROM CUSTOMER;

SELECT INITCAP('mahmuda rawnak jahan') from dual;

SELECT INSTR('Tech on the net','e',5,2) FROM DUAL;


SELECT CUSTOMER_NAME FROM CUSTOMER WHERE LENGTH(CUSTOMER_NAME)>=7 AND CUSTOMER_NAME LIKE 'W%';

SELECT CUSTOMER_NAME,CUSTOMER_CITY FROM CUSTOMER WHERE UPPER(CUSTOMER_CITY) = 'HARRISON';

SELECT RPAD('Mahamuda Rowanak',LENGTH('Mahamuda Rowanak')+6, ' Jahan') FROM DUAL;

SELECT LPAD(CUSTOMER_CITY,LENGTH(CUSTOMER_CITY)+5,'City-') FROM CUSTOMER;

SELECT  LTRIM(RTRIM('xyxzyyyTechxyx','xyz'),'xyz')FROM DUAL;
...
//median,mod,power,round,trunc,sort, DATE
